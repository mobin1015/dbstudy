...
SELECT E.EMP_NO, E.NAME, E.DEPART, D.DEPT_NAME
  FROM DEPARTMENT_T D INNER JOIN EMPLOYEE_T E
    ON D.DEPT_NO = E.DEPART;                     --ANSI문법, 기본키가 있는 테이블을 앞에다 두면 성능이 더 좋다.
...
/* 
    ...
    2. 데이터를 조회하고자 하는 테이블이 왼쪽에 있으면 왼쪽 외부 조인이고 오른쪽에 있으면 오른쪽 외부 조인이다.
    3. ANSI 문법
        1) 왼쪽 외부 조인
        SELECT 조회할 칼럼, ...
          FROM 테이블1 LEFT [OUTER](생략가능) JOIN 테이블2
            ON 조인조건
