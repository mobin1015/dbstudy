...
CREATE SEQUENCE DEPT_SEQ
    ...
    NOMINVALUE     -- 하항선 없음
    NOCYCLE        -- 번호 순환 없음
    CACHE 20       -- 20개의 번호 미리 생성 (번호를 미리 생성하지 않는 NOCACHE옵션을 사용함)
    NOORDER        -- 번호를 순서 없이 사용
    ;
    
-- 사원 테이블의 사원번호를 생성하는 시퀀스 만들기
CREATE SEQUENCE EMPOLY_SEQ
    START WITH 10001
    NOCACHE;
...
-- 사원 테이블에 행 삽입하기
INSERT INTO EMPLOYEE_T VALUES(EMPOLY_SEQ.NEXTVAL, '구창민',1 ,'과장','M','95/05/01',5000000);
INSERT INTO EMPLOYEE_T VALUES(EMPOLY_SEQ.NEXTVAL, '김민서', 1,'사원','M','17/09/01',2500000);
INSERT INTO EMPLOYEE_T VALUES(EMPOLY_SEQ.NEXTVAL, '이은영', 2,'부장','F','90/09/01',5500000);
INSERT INTO EMPLOYEE_T VALUES(EMPOLY_SEQ.NEXTVAL, '한성일', 2,'과장','M','93/04/01',5000000);
...
-- 3. DEPARTMENT_T에서 부서번호(DEPT_NO)가 2인 부서를 삭제하시오. (부서번호가 2인 사원이 있다. 부서가 없어지면 사원 정보가 첨조 무결성에 위배된다. 참조 무결성 위배에 대비해서 ON DELETE SET NULL 옵션을 주었으므로 사원 테이블의 부서번호가 NULL로 함께 수정된다.)
DELETE 
  FROM DEPARTMENT_T 
 WHERE DEPT_NO =2;
