...
/*
    테이블 수정하기
    1. 칼럼 추가하기      : ALTER TABLE 테이블명 ADD            칼럼명  데이터탑입 제약조건
...
-- 1. BANK_T에 연락처(BANK_TELL) 칼럼을 추가하시오.
     ALTER TABLE BANK_T ADD BANK_TELL VARCHAR2(15 BYTE );
...
-- 3. CUSTOMER_T의 나이(CUST_AGE) 칼럼을 삭제하시오.
     ALTER TABLE COUSTOMER_T DROP COLUMN CUST_AGE;

-- 4. CUSTOMRT_T의 연락처(CUST_PHONE) 칼럼명을 CUST_TEL로 수정하시오.
     ALTER TABLE CUSTOMER_T RENAME COLUMN CUST_PHONE TO CUST_TEL;

-- 5. CUSTOMER_T에 등급(GRADE) 칼럼을 추가하시오. 'VIP','SILVER', 'GOLD', 'BRONZE' 중 하나의 값을 가지도록 설정하시오.
     ALTER TABLE CUSTOMER_T ADD GRADE VARCHAR2(6 BYTE) CHECK(GRADE = 'VIP' OR GRADE = 'GOLD' OR GRADE = 'BRONZE' OR GRADE = 'SILVER');
                                                    -- CHECK(GRADE IN('VIP','GOLD','BRONZE'));  
...
/*
    ...
    1. PK
        ...
        2) 삭제
            ...
            (2) ALTER TABLE 테이믈병 DROP PRIMARY KRY
...
-- FK_CUSTOMER_BANK 조약조건을 비활성화하시오.
