...
SELECT EMPLOYEE_ID AS 사번,
     CEIL(MONTHS_BETWEEN('23/12/29' ,HIRE_DATE))-1 AS 근무개월수
  FROM EMPLOYEES
 ...
 -- AC_MGR     AC     MGR
 SELECT  JOB_ID, SUBSTR(JOB_ID,1,2) AS 분야, SUBSTR(JOB_ID, 4) AS 직급
   FROM EMPLOYEES;
...
-- 1        102   01/01/13
-- 2        206   02/06/07
SELECT ROW_NUMBER() OVER(ORDER BY HIRE_DATE ASC) AS 입사순위, EMPLOYEE_ID AS 사번, HIRE_DATE AS 고용일
  FROM EMPLOYEES;
