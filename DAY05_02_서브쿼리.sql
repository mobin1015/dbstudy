...
/*
    단일 행 서브 쿼리 (실행결과와 상관없이 밑의 조건을 만족한다면 단일 행)
...
SELECT *
 ...
 WHERE DEPART IN (SELECT DEPT_NO 
                   FROM DEPARTMENT_T
                  WHERE DEPT_NAME ='영업부'); --DEPT_NAME은 PK도 UNIQUE도 아니기 때문에 잠재적 다중행이다.
...
/*
    ...
    2. 서브 쿼리의 실행 결과를 일종의 임시 테이블의 형식으로 FROM 절에 두고 사용한다.
    3. SELECT 문의 실행 순서를 조정할 때 사용할 수 있다.
*/

-- 1. 2번째로 입사한 사원을 조회하시오.
-- 1) HIRE-DATE의 오름차순 정렬을 한다. (입사순 정렬)
-- 2) 오름차순 결과에 행 번호(ROWNUM)를 붙인다.       - 별명 A (ROWNUM은 가상 칼럼이기 때문에 이를 이용해 값을 조회하려면 AS를 이용해 별명을 붙여주고 별명으로 조회해야 한다)
-- 3) 행 번호가 2인 데이터를 조회한다.                - 별명 B
SELECT B.*
...
-- 1) HIRE-DATE의 오름차순 정렬을 하고 행 번호(ROW_NUMBER 함수)를 붙인다.
...
AS (SELECT DEPT_NO, DEPT_NAME, LOCATION
      FROM DEPARTMENT_T
     WHERE 1=2); --False 값을 조건에 넣으면 빈테이블이 생성
     

/*
    INSERT 문과 서브 쿼리
    1. 서브 쿼리의 결과를 INSERT 할 수 있다.
    2. 한 번의 여러 행을 INSERT 할 수 있다.
    3. 형식
